...
async fn test_context_attributes() {
    ...
    let value: Option<String> = ctx.get_attribute("key1").await;
    ...
    let value: Option<i32> = ctx.get_attribute("key2").await;
    assert_eq!(value, None);
}
...
async fn test_context_route_params() {
    ...
    let id: OptionString = ctx.get_route_param("id").await;
    assert_eq!(id, Some("123".to_string()));
    let name: OptionString = ctx.get_route_param("name").await;
    assert_eq!(name, None);
}
